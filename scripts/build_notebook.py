...
CELLS = [
    ...
    md("## Часть 2. Математика и архитектура DCGAN\n\nМинимаксная игра, оптимальный "
    ...
    md("## Часть 3. Базовое обучение и оценка\n\nЖивая генерация из обученного "
    ...
    md("Базовая модель даёт узнаваемую одежду; D(x) и D(G(z)) держатся около "
       "устойчивого равновесия. Проверка запоминания: генерации не ближе к train, "
       "чем реальные тестовые объекты."),
    md("## Часть 4. Нарушение данных: дисбаланс и отравление\n\nВариант A — удаление "
    ...
    md("При дисбалансе падает recall и доля редких классов в генерациях при "
    ...
    md("Латентное пространство генератора гладко интерполирует между объектами; "
